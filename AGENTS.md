...
- `apps/api`: the NestJS backend. It serves `/api/*` (state, moments, spaced retrieval), Twilio WhatsApp at `/whatsapp`, and the family record for the website at `/web/*` (`apps/api/src/app/web.ts`, checks a Telegram Login `id_token` on each request), and calls Gemini. Auth is Nest JWT Bearer (`AUTH_JWT_SECRET`) with accounts in Neon Postgres (`eu-central-1`). Neon also holds file metadata (`user_files`) and optional moment snapshots. Media uses Vercel Blob (`fra1`). Deploy: Vercel `anchor-api` (see `DEPLOY.md`). Secrets: `apps/api/.env.local` / `.env.example`. Root uses pnpm and Nx: `pnpm dev:api`, `pnpm dev:web`, `pnpm lint`, `pnpm typecheck`, `pnpm nx test api`.
- `apps/web`: the Astro + React website (landing, legal pages, Telegram registration, the family record at `/family`, data export and deletion at `/my-data`, interactive `/app` demo). Registration uses Telegram Login: the page keeps the OpenID Connect `id_token` in `sessionStorage` and sends it only to the bot at `PUBLIC_BOT_URL` (`apps/web/src/lib/telegram.ts`). The website does not call `anchor-api`. The bot's `/web/*` API is designed in `docs/specs/2026-09-26-website-registration-with-telegram.md` and is not built yet. Static build on Vercel project `anchor`. Local: `pnpm dev:web`. Env: `apps/web/.env.example` (`PUBLIC_TELEGRAM_CLIENT_ID`, `PUBLIC_BOT_URL`).
