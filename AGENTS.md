...
- `apps/web`: the Astro + React website (landing, legal pages, signup/login, interactive `/app` demo). Signup/login use Nest `/api/auth/*` JWTs (localStorage). Static build on Vercel project `anchor`. Local: `pnpm dev:web` (proxies `/api` to the API). Env: `apps/web/.env.example`.
