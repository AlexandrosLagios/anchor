...
- `apps/api`: the NestJS backend. It serves `/api/*` (state, moments, spaced retrieval), Twilio WhatsApp at `/whatsapp`, and calls Gemini. Auth is Nest JWT Bearer (`AUTH_JWT_SECRET`) with accounts in Neon Postgres (`eu-central-1`). Neon also holds file metadata (`user_files`) and optional moment snapshots. Media uses Vercel Blob (`fra1`). Deploy: Vercel `anchor-api` (see `DEPLOY.md`). Secrets: `apps/api/.env.local` / `.env.example`. Root uses pnpm and Nx: `pnpm dev:api`, `pnpm dev:web`, `pnpm lint`, `pnpm typecheck`, `pnpm nx test api`.
