...
- `apps/api`: the NestJS backend. It serves `/api/*` (state, moments, spaced retrieval), Twilio WhatsApp at `/whatsapp`, and calls Gemini. Auth is Firebase ID token Bearer verification (Admin SDK + ADC / `FIREBASE_PROJECT_ID=a11y-hack26ath-267`). Account profiles/consents live in Firestore (`eur3`); Neon Postgres (`eu-central-1`) holds file metadata (`user_files`) and optional moment snapshots. Deploy: Vercel `anchor-api` (see `DEPLOY.md`). Secrets: `apps/api/.env.local` / `.env.example`. Root uses pnpm and Nx: `pnpm dev:api`, `pnpm dev:web`, `pnpm lint`, `pnpm typecheck`, `pnpm nx test api`.
- `apps/web`: the Astro + React website (landing, legal pages, signup/login, interactive `/app` demo). Firebase client Auth (email/password); static build on Vercel project `anchor`. Local: `pnpm dev:web` (proxies `/api` to the API). Env: `apps/web/.env.example` (`PUBLIC_FIREBASE_*`).
