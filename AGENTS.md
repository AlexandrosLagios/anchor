...
- `apps/api`: the NestJS backend. It serves `/api/*` (state, moments, spaced retrieval, auth), Twilio WhatsApp at `/whatsapp`, and calls Gemini. Auth is email/password + JWT; user data lives in Neon Postgres (`eu-central-1`) via the Vercel Marketplace. Deploy: Vercel `anchor-api` (see `DEPLOY.md`). Secrets: `apps/api/.env.local` / `.env.example`. Root uses pnpm and Nx: `pnpm dev:api`, `pnpm dev:web`, `pnpm lint`, `pnpm typecheck`, `pnpm nx test api`.
- `apps/web`: the Astro + React website (landing, legal pages, signup/login, interactive `/app` demo). Static build on Vercel project `anchor`. Local: `pnpm dev:web` (proxies `/api` to the API). Env: `apps/web/.env.example`.
