...
- A named request, such as "Anchor, I want a memory of Lucy", is a `memory` intent. The intent call returns `momentIds`: every moment about the person, the pet, the place, or the activity that the message names, from the titles and the tags. The moment list of the intent prompt shows the tags of each moment. The prompt asks the model to add a moment that names the same pet or person only by a general word, such as "dog", but the model does so only some of the time. The collection holds the picked moments, with the label `fromRecord`.
...
- A memory request needs no "Anchor," in the group, but Anchor must never become annoying (section 1):
  - A message without "Anchor," reaches the intent call only when it holds a memory word: memory, memories, photo, picture, pics, moment, album, remember, "show me", or "show us". A reply to another message and a forwarded message never do.
  ...
  - In a probe on the real model on 2026-09-26, 7 requests and 5 ordinary messages ran twice each. 12 of the 14 request runs posted the album, and none of the 10 ordinary runs got an answer. "Show us Lucy" without "Anchor," got no answer.
